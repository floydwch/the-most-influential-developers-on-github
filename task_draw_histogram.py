...
def main(field):
    ...
    products_sqrts = map(math.sqrt, list(starmap(
        operator.mul, zip(followers_numbers, stars_numbers))))
    ...
    plt.bar(index + bar_width, stars_numbers, bar_width, color='b',
            label='Starred')
    plt.bar(index + 2 * bar_width, products_sqrts, bar_width, color='y',
            label='Product')

    plt.xlabel('Developer')
    plt.ylabel('Number')
    plt.title("Developer's Followers and Starred")
    plt.xticks(index + 2 * bar_width, ranks)
    plt.legend()
    plt.tight_layout()
